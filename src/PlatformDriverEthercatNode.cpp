...
void PlatformDriverEthercatNode::updateFtsReadings()
{
    ...
    for (const auto& fts_params : fts_mapping_)
    {
        ...
        geometry_msgs::msg::WrenchStamped& wrench(fts_readings_.wrenches[i]);
...
void PlatformDriverEthercatNode::updateTempReadings()
{
    ...
    for (const auto& joint : active_joint_mapping_)
    {
        ...
        sensor_msgs::msg::Temperature& temperature(temp_readings_.temperatures[i]);

        temperature.header.stamp = this->now();
        ...
        if (first_window)
        {
            temperature.temperature = temp_sums[i] / (1.0 * (temp_index + 1));
        }
        else
        {
            temperature.temperature = temp_sums[i] / (1.0 * window_size);
        }
